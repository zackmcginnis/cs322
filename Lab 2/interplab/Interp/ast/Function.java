...
public class Function extends Defn {
    ...
    /** Run a program with this function as the entry point
     *  and with enough space for the given number of global
     *  variables.
     */
    public void run(Defn[] defns, int numGlobals) {
        // Initialize global variables:
        int[] globals = new int[numGlobals];
        for (int i=0; i<defns.length; i++) {
            defns[i].initialize(globals);
        }

        // Run main program:
        call(new Expr[0], globals, null);
    }

    /** Run initialization code on this definition.
     */
    void initialize(int[] globals) {
        // No initialization required for functions
    }

    /** Execute a call to this function with the given arguments,
     *  and the specified memories for global and local variables,
     *  passing back the return value of the function (if any) as
     *  a result.
     */
